...
exports.getOutUserbyDate = async (req, res) => {
    ...
    try {
        ...
        const userData = await Register.find({
            outTime: {
                $gte: new Date(year, month, day,0,0,0), // Greater than or equal to the start of the day
                $lt: new Date(year, month, day ,23,59,59) // Less than the start of the next day
            }
        }).sort({ outTime:-1 });
